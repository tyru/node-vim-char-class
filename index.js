...
function getCharClass(s, bigWord) {
  ...
  if (cp === 0x20 || cp === 0x9) // ' ' || '\t'
    return CLASS_WHITESPACE;
  // if (enc_dbcs != 0 && cp > 0xFF) {
  //   /* If bigWord, report multi-byte chars as class CLASS_PUNCTUATION. */
  //   if (enc_dbcs == DBCS_KOR && bigWord)
  //     return CLASS_PUNCTUATION;
  //
  //   /* process code leading/trailing bytes */
  //   return dbcs_class(((unsigned)cp >> 8), (cp & 0xFF));
  // }
  // if (enc_utf8) {
  ...
  /* If bigWord is TRUE, report all non-blanks as class CLASS_PUNCTUATION. */
  // if (bigWord)
  //   return CLASS_PUNCTUATION;
  //
  // if (isWordChar(cp))
...
function isWordChar(cp) {
  // if (cp >= 0x100) {
  //   if (enc_dbcs != 0)
  //     return dbcs_class((unsigned)cp >> 8, (unsigned)(cp & 0xff)) >= 2;
  //   if (enc_utf8)
  //     return utf_class_buf(cp, buf) >= 2;
  //   return FALSE;
  // }

  // TODO
  // return (cp > 0 && GET_CHARTAB(buf, cp) != 0);
